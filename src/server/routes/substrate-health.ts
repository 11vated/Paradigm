        ...
        try {
          ...
          try {
            ...
            part6 = {
              ...
              federation: `Fed v1 REAL 2-node + multi-node (3-node demo, hardened Consolidation): ${realFed.claim}; multi=${multiFed?.claim || 'n/a'}; verified=${vReal.sigOk && vReal.merkleOk} roundtrip=${(multiFed as any)?.roundtripVerified || false} merge=${!!mergeRes} fork=${!!forkRes.forkedSeedId} lineageLen=${realFed.lineage.length}${(multiFed as any)?.exchangeAB?.richPreview ? ' +richPreview+C2PA' : ''}${(multiFed as any)?.conflictResolved ? ' conflict-resolved' : ''} ledger=${(realFed as any).ledger ? (realFed as any).ledger.length : 0}; sovereignty/index canonical + federation routes (real ECDSA offer/accept + ledger + rich sov)`,
              governance: 'canon-stewardship + hooks + waiver registry',
