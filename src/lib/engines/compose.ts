...
export interface ComposeOptions {
  ...
  name?: string;
  /**
   * Per-stage kind override. When provided, the i-th engine in the chain
   * receives `kind: kinds[i]` as its request kind. Length must equal the
   * number of engines in the chain.
   */
  kinds?: string[];
  /**
   * Per-stage outputPath override. When provided, the i-th engine in the
   * chain receives `outputPath: outputPaths[i]` as its request outputPath.
   * Length must equal the number of engines in the chain. Required when
   * stage engines have incompatible outputPath expectations (e.g. one
   * wants a directory, the next wants a file path with `.json` extension).
   */
  outputPaths?: string[];
  /**
...
export function composeWithOptions(
  ...
  const composedEngine: Engine = Object.freeze({
    ...
    generate: async (req: unknown, ctx?: EngineContext): Promise<unknown> => {
      ...
      if (opts.kinds && opts.kinds.length !== engines.length) {
        throw new Error('compose: kinds.length must equal engines.length');
      }
      if (opts.outputPaths && opts.outputPaths.length !== engines.length) {
        throw new Error('compose: outputPaths.length must equal engines.length');
      }
      for (let i = 0; i < engines.length; i++) {
        const eng = engines[i];
        let reqForStage: MinimalRequest = current;
        if (opts.kinds && opts.kinds[i] !== undefined) {
          reqForStage = { ...reqForStage, kind: opts.kinds[i] };
        }
        if (opts.outputPaths && opts.outputPaths[i] !== undefined) {
          reqForStage = { ...reqForStage, outputPath: opts.outputPaths[i] };
        }
        const out = (await eng.generate(reqForStage as never, effectiveCtx)) as MinimalArtifact;
