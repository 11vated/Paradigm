...
export interface ComposeOptions {
  /**
   * Optional shared output root. When set, each stage's outputPath is
   * auto-derived via normalizeForEngine(engineId, kind, <root>/<stage-id>),
   * removing the need to thread outputPaths manually.
   */
  outputRoot?: string;
  ...
  const composedEngine: Engine = Object.freeze({
    ...
    generate: async (req: unknown, ctx?: EngineContext): Promise<unknown> => {
      ...
      for (let i = 0; i < engines.length; i++) {
        const eng = engines[i];
        if (opts.outputRoot && typeof (current as { kind?: string }).kind === 'string') {
          const stageDir = path.join(opts.outputRoot, eng.capability.id + ':' + i);
          const normalized = normalizeForEngine(
            eng.capability.id as EngineId,
            (current as { kind: string }).kind,
            stageDir,
          );
          current = { ...current, outputPath: normalized } as MinimalRequest;
        }
