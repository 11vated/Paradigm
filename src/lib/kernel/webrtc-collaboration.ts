   ...
   on(event: string, handler: (...args: any[]) => any): void {
     switch (event) {
       ...
       case 'breed-requested':
         this.onBreedRequested = handler as (parent1: Seed, parent2: Seed, fromPeer: string) => void;
         break;
       case 'evolve-update':
         this.onEvolveUpdate = handler as (data: any, fromPeer: string) => void;
         break;
       case 'user-joined':
         this.onUserJoined = handler as (userId: string, name: string) => void;
         break;
       case 'user-left':
         this.onUserLeft = handler as (userId: string) => void;
         break;
     }
   }
