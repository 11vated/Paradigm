...
async function growGeometry3d(seed: Seed): Promise<Artifact> {
  ...
  const outputPath = `${outputDir}/${fileName}`;
    ...
    try {
      ...
      const result = await generateGeometry3D(seed, outputPath) as any;
      return {
...
async function growAlife(seed: Seed): Promise<Artifact> {
  ...
  try {
    const result = await generateAlifeWorker(seed as any);
    return {
      ...
      artifact: { filePath: result.filePath, format: 'JSON', entityCount: (result as any).entityCount, workerScript: true },
