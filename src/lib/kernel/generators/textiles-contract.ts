...
async function synthesize(seed: S): Promise<A> {
  ...
  const r = await withKernelClock(0, () => generateTextiles(seed as any, out)) as { filePath?: string; designPath?: string };
  const filePath = r.filePath ?? out;
  const data = await fsp.readFile(filePath, 'utf-8').catch(async () => (await fsp.readFile(filePath)).toString('base64'));
  let parsed: any = {};
  try { parsed = JSON.parse(data); } catch { /* fallback */ }
  const summary = `Textiles ${parsed.textiles?.fabric || '?'} ${parsed.textiles?.pattern || ''} durability ${parsed.properties?.durability?.toFixed?.(2) || 'n/a'}.`;
  const metrics: Record<string, number> = {
    durability: parsed.properties?.durability || 0,
