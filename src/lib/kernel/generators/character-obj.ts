...
interface BodyProportions {
  ...
  torsoHeight: number;      // Torso height (for GPU)
 ...
 */
export function extractParams(seed: Seed, rng: Xoshiro256StarStar): CharacterParams {
  const quality = ((seed.genes?.quality?.value as string) || 'medium') as CharacterParams['quality'];
  ...
  const bodyFactor = bodyType === 'slim' ? 0.85 : bodyType === 'athletic' ? 1.0 : bodyType === 'heavy' ? 1.15 : 1.0;
   ...
   const proportions: BodyProportions = {
    ...
    torsoHeight: baseHeight * 0.35, // Add torsoHeight to match BodyProportions interface
