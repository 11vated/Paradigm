...
async function synthesize(seed: S): Promise<A> {
  ...
  const summary = `Hospitality ${parsed.hospitality?.type || 'venue'} in ${parsed.hospitality?.location || 'location'}. Capacity: ${parsed.operations?.capacity || '?'}`;
  const metrics: Record<string, number> = {
    capacity: parsed.operations?.capacity || 0,
    occupancy: parsed.operations?.occupancyRate || 0,
    revenue: parsed.economics?.projectedRevenue || 0
  };
