...
async function synthesize(seed: S): Promise<A> {
  ...
  const summary = `Smart home ${parsed.smartHome?.layout || 'layout'} with ${parsed.smartHome?.devices || '?'} devices. Energy: ${parsed.performance?.energyEfficiency?.toFixed?.(2) || 'n/a'}`;
  const metrics: Record<string, number> = {
    devices: parsed.smartHome?.devices || 0,
    energy: parsed.performance?.energyEfficiency || 0,
    automation: parsed.performance?.automationLevel || 0
  };
