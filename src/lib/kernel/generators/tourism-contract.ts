...
async function synthesize(seed: S): Promise<A> {
  ...
  const summary = `Tourism ${parsed.tourism?.type || '?'} in ${parsed.tourism?.location || '?'} for ${parsed.tourism?.visitors || '?'} visitors. Revenue: ${parsed.economics?.revenue?.toFixed?.(0) || 'n/a'}`;
  const metrics: Record<string, number> = {
    visitors: parsed.tourism?.visitors || 0,
    revenue: parsed.economics?.revenue || 0,
    satisfaction: parsed.performance?.satisfaction || 0
  };
  const previewData = data;
  return {
    filePath: data,
    meta: { brochurePath: richPath },
    previewData,
