...
async function synthesize(seed: S): Promise<A> {
  ...
  const summary = `Transportation ${parsed.transportation?.mode || 'system'} for ${parsed.transportation?.route || 'route'}. Capacity: ${parsed.operations?.capacity || '?'}`;
  const metrics: Record<string, number> = {
    capacity: parsed.operations?.capacity || 0,
    efficiency: parsed.performance?.efficiency || 0,
    cost: parsed.economics?.costPerKm || 0
  };
