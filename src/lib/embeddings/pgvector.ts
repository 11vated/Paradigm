...
export class PgVectorStore {
  ...
  constructor(private config: PgVectorStoreConfig = {}) {
  ...
  async connect(): Promise<void> {
    ...
    const host = process.env.POSTGRES_HOST || this.config.host || 'localhost';
    const port = parseInt(process.env.POSTGRES_PORT || this.config.port?.toString() || '5432');
    const database = process.env.POSTGRES_DB || this.config.database || 'paradigm';
    const user = process.env.POSTGRES_USER || this.config.user || 'paradigm';
    const password = process.env.POSTGRES_PASSWORD || this.config.password || 'paradigm';

    Object.assign(pgConfig, {
      host: connectionString?.replace(/.*@/, '').replace(/:.*/, '') || host,
      port,
      database,
      user,
      password,
