  ...
  generateChordProgression(
    ...
    const progressionPattern = progression[rng.nextInt(0, progression.length - 1)];
      ...
      for (let m = 0; m < measures; m++) {
        ...
        for (let n = 0; n < progressionPattern.length; n++) {
          const chord = progressionPattern;
...
export function synthesizeMusic(seed: {
  ...
  const rng = seed.$hash ? rngFromHash(seed.$hash) : new Xoshiro256StarStar(Date.now().toString());
