...
export default function PromptBar({ onSeedCreated, value: externalValue, onChange: externalOnChange }: { onSeedCreated: any; value: any; onChange: any }) {
  const [internalPrompt, setInternalPrompt] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const generateNewSeed = useSeedStore((s: any) => s.generateNewSeed);
