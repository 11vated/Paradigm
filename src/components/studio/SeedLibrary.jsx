    ...
    <div className="p-3 space-y-3" data-testid="seed-library">
      ...
      <div className="flex flex-wrap gap-1">
        <button onClick={() => { setSelectedDomain('all'); setShowSimilar(false); }}
          className={`px-2 py-0.5 font-mono text-[8px] uppercase tracking-wider border transition-colors ${!showSimilar && selectedDomain === 'all' ? 'border-primary text-primary' : 'border-neutral-800 text-neutral-600 hover:border-neutral-700'}`}
        ...
        {activeSeed && (
          <button onClick={handleFindSimilar}
            className={`ml-auto px-2 py-0.5 font-mono text-[8px] uppercase tracking-wider border transition-colors flex items-center gap-1 ${showSimilar ? 'border-primary text-primary bg-primary/10' : 'border-neutral-800 text-neutral-400 hover:border-primary hover:text-primary'}`}
            ...
            return (
              <div key={i} className="flex items-center gap-2 p-2 bg-black/20 hover:bg-black/40 transition-colors group" data-testid={`library-seed-${i}`}>
                ...
                <button onClick={() => handleImport(seed)} disabled={importing === seed.$name}
                  className="opacity-0 group-hover:opacity-100 px-2 py-0.5 bg-primary text-black font-bold text-[8px] uppercase tracking-wider hover:bg-primary/80 transition-all flex items-center gap-1"
