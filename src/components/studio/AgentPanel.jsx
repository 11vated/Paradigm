...
function MessageBubble({ role, content, intent, data }) {
  ...
  return (
    <div className={`flex gap-2 ${isAgent ? '' : 'flex-row-reverse'}`}>
      <div className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${isAgent ? 'bg-orange-500/10 text-orange-500' : 'bg-neutral-800 text-neutral-400'}`}>
        {isAgent ? <Bot className="w-3 h-3" /> : <User className="w-3 h-3" />}
      </div>
      <div className={`max-w-[85%] space-y-1 ${isAgent ? '' : 'text-right'}`}>
        <div className={`inline-block px-3 py-2 font-mono text-[11px] leading-relaxed ${isAgent ? 'bg-neutral-900/60 border border-neutral-800/50 text-neutral-300' : 'bg-orange-500/10 border border-orange-500/20 text-neutral-200'}`}>
          {content}
        </div>
        {intent && intent !== 'error' && (
          <div className="font-mono text-[8px] text-neutral-600 flex items-center gap-1">
            ...
            intent: {intent}
            {data?.seed && <span className="text-orange-500/60 ml-1">+ seed created</span>}
            {data?.seeds && <span className="text-orange-500/60 ml-1">+ {data.seeds.length} seeds</span>}
            {data?.population && <span className="text-emerald-500/60 ml-1">+ pop:{data.population.length}</span>}
            {data?.domains && <span className="text-cyan-500/60 ml-1">{data.domains.length} domains</span>}
            {data?.types && <span className="text-purple-500/60 ml-1">{data.types.length} types</span>}
          </div>
        )}
      </div>
    </div>
    ...
    <div className="flex flex-col h-full" data-testid="agent-panel">
      ...
      <div className="px-3 py-2 border-b border-neutral-800/50 flex items-center gap-2">
        ...
        <span className="font-mono text-[8px] text-neutral-700 ml-auto flex items-center gap-1">
          {wsConnected ? (
            <><Wifi className="w-2.5 h-2.5 text-emerald-500" /> ws</>
          ) : (
