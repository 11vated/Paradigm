...
export class ArtFunctor implements FunctorBridge {
  ...
  encode(seed: UniversalSeed): number[] {
    return [
      extractNumber(seed.getGeneValue(GeneType.COLOR), 0),
      extractNumber(seed.getGeneValue(GeneType.SHAPE), 0),
      extractNumber(seed.getGeneValue(GeneType.TEXTURE), 0),
      extractNumber(seed.getGeneValue(GeneType.MATERIAL), 0.5),
      extractNumber(seed.getGeneValue(GeneType.LIGHTING), 0.3)
    ];
  }
...
export class FunctorRegistry {
  ...
  getDomainNames(): string[] {
    return Array.from(new Set(this.functors.values().map(f => f.domain)));
  }

  getByDomain(domain: string): FunctorBridge[] {
    return this.functors.values().filter(f => f.domain === domain).toArray();
  }
}
