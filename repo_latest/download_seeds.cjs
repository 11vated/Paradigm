...
const TARGET_DIR = path.join(__dirname, 'data', 'seed-commons');
...
async function main() {
  ...
  const files = treeData.tree.filter(t => t.path.startsWith('seed-commons/') && t.path.endsWith('.gspl'));
  ...
  console.log('Done downloading seeds.');
}
