...
export default function CompositionPanel({ seed, gallery, onComposed }) {
      ...
      {!seed ? (
        ...
        <>
          <div className="p-2 border border-neutral-900 bg-black/30">
            ...
            <div className="font-mono text-[10px] text-orange-500">{seed.$domain}</div>
          ...
          {path && (
            <div className="p-2 border border-cyan-500/20 bg-cyan-500/5">
              <div className="font-mono text-[8px] text-cyan-500 uppercase mb-1">Composition Path</div>
              {path.path ? (
                <div className="flex items-center gap-1 flex-wrap">
                  {path.path.map((step, i) => (
                    <span key={i} className="font-mono text-[9px]">
                      ...
                      <span className="text-cyan-500/60 text-[8px] ml-0.5">({step[2]})</span>
                    </span>
                  ))}
                  <span className="text-neutral-700 mx-1">→</span>
                  <span className="text-emerald-500 font-mono text-[9px]">{targetDomain}</span>
                </div>
              ) : (
          ...
          <div className="flex gap-2">
            ...
            <button data-testid="compose-btn" onClick={handleCompose} disabled={loading}
              className="flex-1 py-1.5 bg-cyan-500 text-black font-bold text-[9px] uppercase tracking-wider hover:bg-cyan-400 transition-colors disabled:opacity-30 flex items-center justify-center gap-1">
