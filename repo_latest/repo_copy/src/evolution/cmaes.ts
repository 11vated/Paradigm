  ...
  private updateDistribution(selected: UniversalSeed[], weights: number[], muEff: number): void {
    ...
    for (let i = 0; i < this.config.dimension; i++) {
      ...
      for (let j = 0; j < weights.length; j++) {
        ...
        sum += weights[j] * (tempSelected[idx].getGeneValue(17) as number[])?.[i] ?? 0;
      }
      newMean[i] = sum;
    }
    ...
    const hs = this.ps.reduce((sum, p) => sum + p * p, 0) < 2 * this.config.dimension * Math.pow(this.config.dimension, 0.5);
