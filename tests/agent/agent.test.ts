...
describe('GSPL Agent', () => {
  describe('intent classification', () => {
    it('classifies "create a character seed" as create_seed', async () => {
      const r = await agent.process('create a character seed');
      expect(r.intent).toBe('create_seed');
      expect(r.success).toBe(true);
    });

    it('classifies "list domains" as list_domains', async () => {
    ...
    it('classifies "list gene types" as list_gene_types', async () => {
      const r = await agent.process('list gene types');
      expect(r.intent).toBe('list_gene_types');
      expect(r.success).toBe(true);
      expect(r.data?.types.length).toBe(17);
    });

    it('classifies "help" as help', async () => {
      const r = await agent.process('help');
      expect(r.intent).toBe('help');
      expect(r.success).toBe(true);
      expect(r.data?.commands).toBeDefined();
    });

    it('classifies mutation queries', async () => {
      const r = await agent.process('mutate this seed');
      expect(r.intent).toBe('mutate_seed');
    });

    it('classifies breeding queries', async () => {
      const r = await agent.process('breed these seeds together');
      expect(r.intent).toBe('breed_seeds');
    });

    it('classifies composition queries', async () => {
      const r = await agent.process('compose to sprite domain');
      expect(r.intent).toBe('compose_seed');
    });

    it('classifies "find path from character to fullgame"', async () => {
      const r = await agent.process('find path from character to fullgame');
      expect(r.intent).toBe('find_composition_path');
    });

    it('classifies GSPL code blocks', async () => {
      const gspl = 'seed "Hero" in character { strength: 80 }';
      const r = await agent.process(gspl);
      expect(r.intent).toBe('parse_gspl');
    });
  });

  describe('seed creation', () => {
    it('creates a seed with domain-appropriate genes', async () => {
      const r = await agent.process('create a new character seed called "Warrior"');
      expect(r.success).toBe(true);
    ...
    it('assigns unique IDs', async () => {
      const r1 = await agent.process('create a character seed');
      const r2 = await agent.process('create a character seed');
      expect(r1.data?.seed.id).not.toEqual(r2.data?.seed.id);
    });

    it('assigns valid hashes', async () => {
      const r = await agent.process('create a sprite seed');
  ...
  describe('GSPL parsing', () => {
    it('detects GSPL code block intent', async () => {
      const r = await agent.process('seed "Dragon" in character { strength: 90 }');
      expect(r.success).toBe(true);
    ...
    it('handles GSPL with categorical values', async () => {
      const r = await agent.process('seed "Knight" in character { archetype: "paladin" }');
      expect(r.success).toBe(true);
      expect(r.intent).toBe('parse_gspl');
    });
  });
  ...
  describe('mutation', () => {
    it('mutates a seed from context', async () => {
      const testSeed = {
        id: 'mut-test',
        $domain: 'character',
        $name: 'Test',
      ...
      const r = await agent.process('mutate this seed', { seeds: [testSeed] });
      expect(r.success).toBe(true);
      expect(r.data?.seed).toBeDefined();
      expect(r.data?.seed.$lineage?.operation).toBe('agent_mutate');
    });

    it('handles mutation with no seeds without crashing', async () => {
      const r = await agent.process('mutate this seed', { seeds: [] });
  ...
  describe('evolution', () => {
    it('evolves a population', async () => {
      ...
      const r = await agent.process('evolve with population 6', { seeds: [testSeed] });
      expect(r.success).toBe(true);
