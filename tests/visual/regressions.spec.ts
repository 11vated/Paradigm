...
const routeReadySelectors: Record<string, string> = {
  '/': 'text=Enter Studio',
  '/studio': '[data-testid="creation-studio"]',
  '/chat': 'text=SeedChat — AI Generative Assistant',
};
...
for (const route of SAMPLE_PAGES) {
  test(`Visual regression: ${route}`, async ({ page }) => {
    ...
    await expect(page.locator(routeReadySelectors[route])).toBeVisible({ timeout: 10000 });
...
test('Studio page loads and renders the core studio UI', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="creation-studio"]')).toBeVisible();
  await expect(page.locator('[data-testid="studio-topbar"]')).toBeVisible();
