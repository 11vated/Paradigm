...
describe('inversePipeline20 + output20Matrix (p24-7/20-21)', () => {
  ...
  it('output20Matrix produces one output per supported modality via compose', async () => {
    ...
    expect(matrix.outputs.length).toBe(27); // one per canonical domain (matches phase21Gate)
  ...
  it('phase20_21 gate helpers report supported counts (preflight uses)', () => {
    ...
    expect(p20.modalitiesSupported).toBe(27);
    expect(p21.outputsSupported).toBe(27);
