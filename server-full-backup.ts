...
// ─── Database Integration (replaces in-memory stores) ───
...
async function initializeDatabase() {
  if (process.env.MONGO_URI) {
    try {
      ...
      const { MongoStore } = await import('./src/lib/data/mongo-store.js');
      mongoStore = new MongoStore(process.env.MONGO_URI, process.env.MONGO_DB_NAME || 'paradigm');
      await mongoStore.init();
      useMongo = true;
      console.log('[Database] MongoDB connected successfully');
    } catch (error) {
      console.error('[Database] MongoDB connection failed, using in-memory fallback:', error);
      useMongo = false;
    }
  } else {
    console.log('[Database] No MONGO_URI provided, using in-memory stores');
  }
}

// In-memory fallback (replaces in-memory stores)
const seeds = new Map<string, any>();
const users = new Map<string, any>();
const auditLog: any[] = [];

// Helper: Get seed store (MongoDB or in-memory)
async function getSeedStore() {
  if (useMongo && mongoStore) {
    return {
      getAll: () => mongoStore.getAllSeeds(),
      get: (id: string) => mongoStore.getSeedById(id),
      set: (id: string, seed: any) => mongoStore.saveSeed(seed),
      delete: (id: string) => mongoStore.deleteSeed(id),
    };
  }
  
  // In-memory implementation
  return {
    getAll: async () => Array.from(seeds.values()),
    get: async (id: string) => seeds.get(id),
    set: async (id: string, seed: any) => seeds.set(id, seed),
    delete: async (id: string) => seeds.delete(id),
  };
}
