...
const API_URL = process.env.REACT_APP_BACKEND_URL || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:8000');
...
export const growSeed = (id) =>
  api.post(`/seeds/${id}/grow`).then(r => r.data);
...
export const verifySeed = (id) =>
  api.post(`/seeds/${id}/verify`).then(r => r.data);
